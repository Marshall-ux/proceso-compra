...
import { crearSolicitud, extraerFactura } from '../services/api.js'
...
const VACIA = {
  ...
  solicitado_por: '', factura_nombre: '', factura_archivo: '', factura_numero: '', items: [],
}
...
export default function NuevaPage() {
  ...
  const procesar = async (archivo) => {
    ...
    setErrores([])
    try {
      const r = await extraerFactura(archivo)
      const leidos = Object.entries(r.datos || {}).filter(([, v]) => v !== '' && v !== 0).map(([k]) => k)
      setDatos({
        ...VACIA,
        ...r.datos,
        items: r.items || [],
        factura_nombre: r.factura_nombre || archivo.name,
        factura_archivo: r.factura_archivo || '',
      })
      setAutocompletados(leidos)
      if (!r.ok) setAviso(r.error)
      else if (leidos.length === 0) setAviso('No se pudo leer ningún dato del PDF. Cargalos a mano.')
      setPaso(2)
    } catch (e) {
      setAviso(e.message)
    } finally {
      ...
      {paso === 1 && (
        <div className="card">
          ...
          <div className="card__hint">
            Leemos los datos del PDF y completamos la autorización genérica. Después revisás todo antes de guardar.
          </div>
          ...
          >
            ...
            <div className="dropzone__title">
              {cargando ? 'Leyendo la factura…' : 'Arrastrá el PDF o hacé clic para elegirlo'}
            </div>
            <div className="dropzone__hint">Un archivo PDF</div>
            <input
      ...
      {paso === 2 && (
        <div className="card">
          ...
          <div className="card__hint">
            ...
            {datos.factura_nombre && <> Factura: <strong>{datos.factura_nombre}</strong>.</>}
          </div>
